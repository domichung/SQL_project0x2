...
node* new_node(string course_id,string data_id) {
    node* p = new node();
...
void insert(node*& root, string course_id, string data_id) {
    if (root == NULL) {
        root = new_node(course_id ,data_id);
    } else {
        node* current = root;
        while (true) {
            if (course_id < current->course_id) {
                if (current->left == NULL) {
                    current->left = new_node(course_id,data_id);
                    break;
                } else {
                ...
                if (current->right == NULL) {
                    current->right = new_node(course_id,data_id);
                    break;
                } else {
...
void in_order_traversal(node* root, ofstream& output_file) {
    if (root != NULL) {
        ...
        string test = root->course_id + " " + root->data_id + "\n";
...
int main(){
    ...
    output_file.open("../all_index/long/sid.txt");
